...
for index, item in enumerate(transactions, start=1):
    ...
    fee_lamports = 0
    try:
        fee_lamports = int(rpc.get_fee_for_message(tx.message).value or 0)
    except Exception as exc:
        print(f"Estimated network fee for Raydium transaction {index}: unavailable ({exc})")
    else:
        print(f"Estimated network fee for Raydium transaction {index}: {fee_lamports} lamports ({fee_lamports / 1_000_000_000:.9f} SOL)")
        print("Estimate excludes token amounts, rent deposits, DEX/platform fees, and later priority-fee changes unless the transaction already includes them.")
    if not env_bool("SEND_TRANSACTION", False):
