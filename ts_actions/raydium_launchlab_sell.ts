...
const sell = await raydium.launchpad.sellToken({
  programId,
  mintA,
...
const feeLamports = await estimateAndPrintTransactionCost((sell as any).transactions ?? (sell as any).transaction, owner);
